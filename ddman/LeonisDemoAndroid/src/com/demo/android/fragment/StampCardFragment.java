...
public class StampCardFragment extends Fragment {
	...
	public boolean onOptionsItemSelected(MenuItem menuitem) {
		...
		case R.id.action_unread:
			...
			((MainActivity) getActivity()).confirm("プレゼント",
					"未取得のプレゼントを取得しますか？", getString(android.R.string.ok),
	...
	public View onCreateView(LayoutInflater layoutinflater,
		...
		Button mButtonMenu0 = (Button) view.findViewById(R.id.btn_menu_0);
		Button mButtonMenu1 = (Button) view.findViewById(R.id.btn_menu_1);
		mButtonMenu0.setVisibility(View.GONE);
		mButtonMenu1.setVisibility(View.GONE);
		...
		mFrameLayout.setLayoutParams(new LayoutParams(
		...
		mStampView
				.setOffersStampCardViewListener(new OffersStampCardView.OffersStampCardViewListener() {
					@Override
					...
					public void afterStamp(Map<String, Object> result) {
						...
						int stamp_id = Integer.parseInt(result.get("stamp_id")
								.toString());
						int x = (int) Float.parseFloat(result.get("x")
								.toString());
						int y = (int) Float.parseFloat(result.get("y")
								.toString());
						int width = (int) Float.parseFloat(result.get("width")
								.toString());
						int height = (int) Float.parseFloat(result
								.get("height").toString());
						int angle = (int) Float.parseFloat(result.get("angle")
								.toString());
						int grid_id = (int) Float.parseFloat(result.get(
								"grid_id").toString());

						mProgressBar.setVisibility(View.VISIBLE);

						mStampCard.stamp(grid_id, x, y, width, height, angle,
								stamp_id, new OffersListener() {
									@Override
									public void onDone(
										...
										mStampCard = (OffersStampCard) result
												.get("stamp_card");

										OffersStatus status = (OffersStatus) result
												.get("status");

										setStampCard();

										final List<OffersStampAction> actions = (List<OffersStampAction>) result
												.get("actions");
	...
	public void onStart() {
		...
		mProgressBar.setVisibility(View.VISIBLE);
		OffersKit.getInstance().stampCard(mStamp_card_id, true,
				new OffersListener() {
					...
					public void onDone(Map<String, Object> map) {
						...
						if (map.get("stamp_card") != null) {
							mStampCard = (OffersStampCard) map
									.get("stamp_card");
							if (mStampCard.hasDetail()) {
								setStampCard();
							} else {
								((MainActivity) getActivity()).alert("onDone",
										"オフラインデータなし");
							}
						}
					}
	...
	private void getPresent() {
		mProgressBar.setVisibility(View.VISIBLE);
		((MainActivity) getActivity()).mStampCardID = mStamp_card_id;
	...
	private void setStampCard() {
		...
		List<Map<String, Object>> json_grids = (mStampCard.getGrids() != null) ? mStampCard
				.getGrids() : null;
		List<Map<String, Object>> json_groups = (mStampCard.getStampGroups() != null) ? mStampCard
				.getStampGroups() : null;
		List<Map<String, Object>> json_histories = (mStampCard.getHistories() != null) ? mStampCard
				.getHistories() : null;

		mStampView.setCard(json_grids, mStampCard.isKeepOrder(), json_groups,
				json_histories);
